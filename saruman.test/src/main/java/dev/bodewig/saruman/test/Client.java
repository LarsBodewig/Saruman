...
public class Client {
  ...
  public static void objectContext() {
    ...
    try {
      saruman =
          new SarumanClient(userId, userStats); // additional argument for custom key file path
  ...
  public static void staticContext() {
    ...
    String code = "unlockCode";
    try {
      SarumanClient.unlockAchievement(
          userId, userStats, code); // additional argument for custom key file path
    } catch (DecryptionException de) {
      // logic for invalid code
      throw new RuntimeException(de);
    } catch (ReadKeyException rke) {
